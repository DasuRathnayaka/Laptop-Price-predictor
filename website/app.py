...
def prediction(lst):
    ...
    with open(filename, 'rb') as file: #to open module. rb means read binary
...
def index():
    ...
    if request.method == 'POST':
        ...
        #create an array as the ML model

        feature_list = []

        feature_list.append(int(ram))
        feature_list.append(float(weight))
        feature_list.append(len(touchscreen)) #if we tick on touchscreen value then send 1 otherwise empty value
